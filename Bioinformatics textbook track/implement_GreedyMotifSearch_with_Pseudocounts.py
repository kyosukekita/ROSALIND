...
file=open('Desktop/Downloads/rosalind_ba2e.txt').read()
...
def FormProfile(motifs):
    profile=[[0.1 for _ in range(4)] for _ in range(len(motifs[0]))]#pseudocountを考慮して、ここを0よりも少しだけ大きくする
    
    for i in range(len(motifs)):
        for j in range(len(motifs[i])):
            if motifs[i][j]=="A":
                profile[j][0]+=1/len(motifs)
            elif motifs[i][j]=="C":
                profile[j][1]+=1/len(motifs)
            elif motifs[i][j]=="G":
                profile[j][2]+=1/len(motifs)
            else:
                profile[j][3]+=1/len(motifs)
